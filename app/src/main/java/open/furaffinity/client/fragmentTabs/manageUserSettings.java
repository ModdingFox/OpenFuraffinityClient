...
public class manageUserSettings extends BaseFragment {
    ...
    protected void initPages() {
        page = new controlsUserSettings(getActivity(), new BasePage.pageListener() {
            @Override
            public void requestSucceeded(BasePage BasePage) {
                if (((controlsUserSettings) BasePage).getAcceptTrades()) {
                    accept_trades_yes.setChecked(true);
                } else {
                    accept_trades_no.setChecked(true);
                }

                if (((controlsUserSettings) BasePage).getAcceptCommissions()) {
                    accept_commissions_yes.setChecked(true);
                } else {
                    accept_commissions_no.setChecked(true);
                }

                uiControls.spinnerSetAdapter(requireContext(), featured_journal_id, ((controlsUserSettings) BasePage).getFeaturedJournalId(), ((controlsUserSettings) BasePage).getFeaturedJournalIdCurrent(), true, false);
    ...
    protected void updateUIElementListeners(View rootView) {
        fab.setOnClickListener(v -> new open.furaffinity.client.submitPages.submitControlsUserSettings(getActivity(), new BasePage.pageListener() {
            @Override
            public void requestSucceeded(BasePage BasePage) {
