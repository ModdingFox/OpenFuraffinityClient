...
public class manageFolderListAdapter
    extends RecyclerView.Adapter<manageFolderListAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v =
            LayoutInflater.from(parent.getContext()).inflate(R.layout.folder_item, parent, false);

        return new ViewHolder(v);
    }

    @Override public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Glide.with(holder.itemView).load(mDataSet.get(position).get("iconLink"))
        ...
        holder.upButton.setOnClickListener(v -> {
            if (mDataSet.get(position).containsKey("upgroup_id")) {
                listener.upButton(mDataSet.get(position).get("upaction"),
                    mDataSet.get(position).get("upkey"), mDataSet.get(position).get("updirection"),
                    mDataSet.get(position).get("upgroup_id"), "group_id");
            }
            else if (mDataSet.get(position).containsKey("upfolder_id")) {
                listener.upButton(mDataSet.get(position).get("upaction"),
                    mDataSet.get(position).get("upkey"), mDataSet.get(position).get("updirection"),
                    mDataSet.get(position).get("upfolder_id"), "folder_id");
            }
        });

        holder.downButton.setOnClickListener(v -> {
            if (mDataSet.get(position).containsKey("downgroup_id")) {
                listener.downButton(mDataSet.get(position).get("downaction"),
                    mDataSet.get(position).get("downkey"),
                    mDataSet.get(position).get("downdirection"),
                    mDataSet.get(position).get("downgroup_id"), "group_id");
            }
            else if (mDataSet.get(position).containsKey("downfolder_id")) {
                listener.downButton(mDataSet.get(position).get("downaction"),
                    mDataSet.get(position).get("downkey"),
                    mDataSet.get(position).get("downdirection"),
                    mDataSet.get(position).get("downfolder_id"), "folder_id");
            }
        });

        holder.deleteButton.setOnClickListener(v -> {
            if (mDataSet.get(position).containsKey("deletegroup_id")) {
                listener.deleteButton(mDataSet.get(position).get("deleteaction"),
                    mDataSet.get(position).get("deletekey"),
                    mDataSet.get(position).get("deletegroup_id"), "group_id");
            }
            else if (mDataSet.get(position).containsKey("deletefolder_id")) {
                listener.deleteButton(mDataSet.get(position).get("deleteaction"),
                    mDataSet.get(position).get("deletekey"),
                    mDataSet.get(position).get("deletefolder_id"), "folder_id");
            }
        });
        ...
        holder.editButton.setOnClickListener(v -> {
            if (mDataSet.get(position).containsKey("editfolder_id")) {
                listener.editButton(mDataSet.get(position).get("editaction"),
                    mDataSet.get(position).get("editfolder_id"));
            }
        });
    }
